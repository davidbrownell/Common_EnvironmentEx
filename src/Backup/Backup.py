...
@CommandLine.EntryPoint(
    destination=CommandLine.EntryPoint.Parameter("Destination directory"),
    input=CommandLine.EntryPoint.Parameter(
        "One or more filenames or directories used to parse for input",
    ),
    force=CommandLine.EntryPoint.Parameter(
        "Ignore information in the destination when calculating work to execute",
    ),
    simple_compare=CommandLine.EntryPoint.Parameter(
        "Compare via file size and modified date rather than with a hash. This will be faster, but more error prone when detecting changes.",
    ),
    include=CommandLine.EntryPoint.Parameter(
        "One or more regular expressions used to specify filenames to include",
    ),
    exclude=CommandLine.EntryPoint.Parameter(
        "One or more regular expressions used to specify filenames to exclude",
    ),
    traverse_include=CommandLine.EntryPoint.Parameter(
        "One or more regular expressions used to specify directory names to include while parsing",
    ),
    traverse_exclude=CommandLine.EntryPoint.Parameter(
        "One or more regular expressions used to specify directory names to exclude while parsing",
    ),
    display_only=CommandLine.EntryPoint.Parameter(
        "Display the operations that would be taken but do not perform them",
    ),
...
@CommandLine.Constraints(
    destination=CommandLine.DirectoryTypeInfo(
        ensure_exists=False,
    ),
    ...
    traverse_include=CommandLine.StringTypeInfo(
        arity="*",
    ),
    traverse_exclude=CommandLine.StringTypeInfo(
        arity="*",
    ),
    output_stream=None,
)
def Mirror(
    ...
    inputs = [FileSystem.Normalize(i) for i in input]
    del input
    includes = include
    del include
    excludes = exclude
    del exclude
    traverse_includes = traverse_include
    del traverse_include
    traverse_excludes = traverse_exclude
    del traverse_exclude
        ...
        with output_stream.DoneManager(
            ...
            if tasks:
                ...
                with dm.stream.SingleLineDoneManager(
                    "Copying {}...".format(inflect.no("file", len(tasks))),
                ) as this_dm:
                    this_dm.result = TaskPool.Execute(
                        [
                            TaskPool.Task(
                                "Copy '{}' to '{}'".format(source, dest),
                                Execute,
                            )
                            for source,
                            dest in tasks
            ...
            if removed_files:
                ...
                with dm.stream.SingleLineDoneManager(
                    "Removing {}...".format(inflect.no("file", len(removed_files))),
                ) as this_dm:
                    this_dm.result = TaskPool.Execute(
                        [
                            TaskPool.Task("Remove '{}'".format(filename), Execute)
...
@CommandLine.EntryPoint(
    ...
    output_dir=CommandLine.EntryPoint.Parameter(
        "Output directory that will contain the file(s) that have been added/removed",
    ),
    input=CommandLine.EntryPoint.Parameter(
        "One or more filenames or directories used to parse for input",
    ),
    force=CommandLine.EntryPoint.Parameter(
        "Ignore previously saved information when calculating work to execute",
    ),
    ssd=CommandLine.EntryPoint.Parameter(
        "Leverage optimizations available if the source drive is a Solid-State Drive (SSD)",
    ),
    compress=CommandLine.EntryPoint.Parameter("Compress the data"),
    encryption_password=CommandLine.EntryPoint.Parameter(
        "Encrypt the data with this password",
    ),
    symlinks=CommandLine.EntryPoint.Parameter(
        "Create symbolic links rather than copying files",
    ),
    ...
    display_only=CommandLine.EntryPoint.Parameter(
        "Display the operations that would be taken but does not perform them",
    ),
    working_dir=CommandLine.EntryPoint.Parameter(
        "Specify a custom working directory; use this option if space on the drive associated with 'output_dir' is limited and another drive is available",
    ),
    no_status=CommandLine.EntryPoint.Parameter(
        "Do not display file-specific status when performing long-running operations",
    ),
...
@CommandLine.Constraints(
    ...
    encryption_password=CommandLine.StringTypeInfo(
        arity="?",
    ),
    include=CommandLine.StringTypeInfo(
        arity="*",
    ...
    hash_block_size=CommandLine.IntTypeInfo(
        min=1,
        arity="?",
    ),
    output_stream=None,
...
def Offsite(
    ...
    if symlinks and (compress or encryption_password):
        raise CommandLine.UsageException(
            "The 'symlinks' option is not compatible with compression or encryption",
        )
        ...
        with output_stream.DoneManager(
            ...
            if not force and os.path.isfile(json_filename):
                try:
                    with open(json_filename) as f:
                        dest_file_info = [
                            _FileInfo.FromJson(item) for item in json.load(f)
                        ]
                    ...
                    dm.stream.write(
                        "WARNING: The previously saved data appears to be corrupt and will not be used.\n",
                    )
            ...
            with dm.stream.DoneManager():
                ...
                for dfi in work.get(None, []):
                    data.append(
                        {"filename": dfi.Name, "hash": dfi.Hash, "operation": "remove"},
                    )
            ...
            with dm.stream.DoneManager(
                ...
                if to_copy:
                    with apply_dm.stream.SingleLineDoneManager(
                        "Copying Content...",
                    ) as copy_dm:
                        ...
                        if symlinks:
                            ...
                            def Execute(task_index, task_output):
                                ...
                                commands = [
                                    CurrentShell.Commands.SymbolicLink(dest, source)
                                    for source,
                                    dest in these_items
                                ]
                            ...
                            tasks = [
                                TaskPool.Task(str(batch), Execute) for batch in batches
                            ]
                            ...
                            def Execute(task_index, on_status_update):
                                ...
                                if not no_status:
                                    on_status_update(
                                        FileSystem.GetSizeDisplay(os.path.getsize(source)),
                                    )
            ...
            with dm.stream.DoneManager():
                ...
                with open(pending_json_filename, "w") as f:
                    json.dump([sfi.ToJson() for sfi in source_file_info], f)

            if auto_commit:
                dm.result = CommitOffsite(
                    backup_name,
...
@CommandLine.EntryPoint(
    ...
    backup_suffix=CommandLine.EntryPoint.Parameter(
        "Suffix used when making a copy of the committed data; this can be helpful when archiving committed data",
    ),
)
@CommandLine.Constraints(
    backup_name=CommandLine.StringTypeInfo(),
    backup_suffix=CommandLine.StringTypeInfo(
        arity="?",
    ),
    output_stream=None,
)
...
# ----------------------------------------------------------------------
@CommandLine.EntryPoint(
    source_dir=CommandLine.EntryPoint.Parameter(
        "Directory that contains all offsite backup output, each iteration stored in a subdirectory",
    ),
    encryption_password=CommandLine.EntryPoint.Parameter(
        "Decrypt the data with this password",
    ),
    dir_substitution=CommandLine.EntryPoint.Parameter(
        "Destination substitutions to perform if the data to be restored should be restored at a location different from when it was backed up",
    ),
    display_only=CommandLine.EntryPoint.Parameter(
        "Display the operations that would be taken but does not perform them",
    ),
...
class _FileInfo(object):
    ...
    def FromJson(cls, item):
        return cls(item["name"], item["size"], item["last_modified"], item["hash"])
    ...
    def AreEqual(
        ...
        return (
            ...
            and abs(self.LastModified - other.LastModified) <= 0.00001
            and (not compare_hashes or self.Hash == other.Hash)
        )
...
def _GetFileInfo(
    ...
    with output_stream.DoneManager() as dm:
        ...
        with dm.stream.DoneManager(
            done_suffix=lambda: "{} found".format(inflect.no("file", len(input_files))),
        ) as this_dm:
            input_dirs = []
            ...
            for i in inputs:
                if os.path.isfile(i):
                    ...
                    raise CommandLine.UsageException(
                        "'{}' is not a valid file or directory".format(i),
                    )
                    ...
                    for index, input_dir in enumerate(input_dirs):
                        ...
                        with dir_dm.stream.DoneManager(
                            done_suffix=lambda: "{} found".format(
                                inflect.no(
                                    "file",
                                    len(input_files) - prev_len_input_files,
                                ),
                            ),
                        ):
                            input_files += FileSystem.WalkFiles(
                                input_dir,
        ...
        if includes or excludes:
            ...
            def ToRegexes(items):
                ...
                for item in items:
                    try:
                        ...
                        raise CommandLine.UsageException(
                            "'{}' is not a valid regular expression".format(item),
                        )
            ...
            with dm.stream.DoneManager(
                lambda: "{} to process".format(inflect.no("file", len(input_files))),
            ):
                if includes:
                    include_regexes = ToRegexes(includes)
                    include_checker_func = lambda input_file: any(
                        include_regex
                        for include_regex in include_regexes
                        if include_regex.match(input_file)
                    )
                else:
                ...
                if excludes:
                    ...
                    exclude_checker_func = lambda input_file: any(
                        exclude_regex
                        for exclude_regex in exclude_regexes
                        if exclude_regex.match(input_file)
                    )
                else:
                ...
                for input_file in input_files:
                    if not exclude_checker_func(input_file) and include_checker_func(
                        input_file,
                    ):
        ...
        if input_files:
            with dm.stream.SingleLineDoneManager("Calculating Info...") as this_dm:
                # ----------------------------------------------------------------------
                ...
                if is_ssd:
                    ...
                    def CreateFileInfoWithHash(filename):
                        ...
                        if info.Size <= hash_block_size * 5:
                            return original_create_file_info_with_hash_func(
                                filename,
                                info=info,
                            )
                ...
                with CallOnExit(Cleanup):
                    ...
                    def CalculateHash(filename, on_status_update):
                        if not no_status:
                            on_status_update(
                                FileSystem.GetSizeDisplay(os.path.getsize(filename)),
                            )
                    ...
                    file_info += TaskPool.Transform(
                        input_files,
                        CalculateHash,
                        this_dm.stream,
                        num_concurrent_tasks=None if is_ssd else 1,
                        name_functor=lambda index,
                        item: item,
                    )
    ...
    with output_stream.DoneManager(
        ...
        if optional_local_destination_dir is None:
            ...
            if IsMultiDrive():
                ...
                # ----------------------------------------------------------------------
                def FromDest(filename):
                    assert filename.startswith(optional_local_destination_dir), (
                        filename,
                        optional_local_destination_dir,
                    )
        ...
        for sfi in six.itervalues(source_map):
            ...
            if dest_filename not in dest_map:
                ...
                dest_map[dest_filename],
                compare_hashes=not simple_compare,
            ):
                matched += 1
            else:
